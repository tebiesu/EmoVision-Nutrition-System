...
- 大范围操作先小范围试验
  ...
  ### 1. 全局原则
    ...
    1. 单轮最多两个 Skills：每轮最多启用两个 Skill；无依赖可并行，有依赖必须串行。
    2. 最小必要：只启用当前任务必需的 Skill，避免“为了用而用”。
    3. 离线优先：优先本地分析与本地代码能力；确需外部能力时再启用对应 Skill。
    4. 失败降级：Skill 不可用或执行失败时，立即切换备选 Skill 或回退通用实现，并标注不确定性。
    5. 可追溯：每次启用 Skill 必须说明“为什么用、做什么、产出什么”。

  ### 2. 启用规则

    1. 用户显式点名 Skill（如 frontend-design）时，必须优先使用该 Skill。
    2. 用户未点名时，按任务意图匹配最合适 Skill；若有多个候选，只选最小集合。
    3. 若 Skill 描述与任务不匹配，不强行使用；直接说明并采用通用流程。
    4. 不跨轮继承 Skill：本轮未提及且任务不匹配时，不默认续用上轮 Skill。

  ### 3. 执行流程

    1. 识别意图：判断任务类型（设计/实现/评审/收尾/技能管理等）。
    2. 选择 Skill：主 Skill 1 个，必要时加 1 个辅助 Skill。
    3. 读取说明：仅读取 SKILL.md 必要部分，避免过度加载上下文。
    4. 执行任务：优先复用 Skill 提供的脚本、模板、资产。
    5. 结果校验：检查输出与需求一致性，给出关键风险和下一步建议。

  ### 4. 选择矩阵（示例）

  - UI/页面实现：frontend-design（备选：ui-ux-pro-max）
  - iOS 界面：mobile-ios-design
  - 仓库理解/架构分析：codebase-analysis
  - 多子任务并行开发：subagent-driven-development
  - 完成后评审：requesting-code-review
  - 分支收尾与集成：finishing-a-development-branch
  - 查找/安装新能力：find-skills、skill-installer
  - 自定义能力沉淀：skill-creator

  ### 5. 并行与冲突控制

    1. 并行任务必须满足“无写冲突”（不同时改同一文件区域）。
    2. 同一文件多处改动：先拆分不重叠区域再并行，否则串行。
    3. 强依赖链任务（A→B→C）必须串行，不强行并行化。
...
从现在开始，**每完成一个独立功能点**，必须立即执行以下流程：

1. 本地提交：
   - `git add` 仅包含该功能点相关文件
   - `git commit -m "<type>: <功能点说明>"`
2. 远程同步：
   - 立即执行 `git push origin <当前分支>`
3. 结果确认：
   - 记录该功能点对应的 commit hash
   - 确认远程分支可见该提交

### 约束要求

1. 禁止将多个无关功能点合并为一次提交。
2. 提交信息必须可追溯到具体功能点与测试结果。
3. 若推送失败，必须先修复后再进入下一个功能点开发。
